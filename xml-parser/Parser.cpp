#include "Parser.h"
#include <iostream>
#include <fstream>
#include <sstream>
using namespace std;
using namespace yazi::xml;

Parser::Parser() : m_str(""), m_idx(0)
{
}

// 通过文件名加载xml文件
bool Parser::load_file(const string& filename)
{
	// 读取文件
	ifstream fin(filename);
	if (fin.fail())
	{
		return false;
	}
	stringstream ss;
	ss << fin.rdbuf();
	m_str = ss.str();
	m_idx = 0;
	return true;
}

// 通过字符串加载xml文件
bool Parser::load_string(const string& str)
{
	m_str = str;
	m_idx = 0;
	return true;
}

// 忽略空格
void Parser::skip_white_space()
{
	while (m_str[m_idx] == ' ' || m_str[m_idx] == '\r' || m_str[m_idx] == '\n' || m_str[m_idx] == '\t')
	{
		m_idx++;
	}
}

Xml Parser::parse()
{
	skip_white_space();
	if (m_str.compare(m_idx, 5, "<?xml") == 0)
	{
		if (!parse_declaration())
		{
			throw logic_error("parse declaration error"); // 解析声明错误
		}
	}
	skip_white_space();
	while (m_str.compare(m_idx, 4, "<!--") == 0) // 判断是否是注释
	{
		if (!parse_comment())
		{
			throw logic_error("parse comment error"); // 解析注释错误
		}
		skip_white_space();
	}
	if (m_str[m_idx] == '<' && (isalpha(m_str[m_idx + 1]) || m_str[m_idx + 1] == '_'))
	{
		return parse_element();
	}
	throw logic_error("parse element error"); // 解析元素错误
}

// 解析xml声明
bool Parser::parse_declaration()
{
	if (m_str.compare(m_idx, 5, "<?xml") != 0)
	{
		return false;
	}
	m_idx += 5;
	size_t pos = m_str.find("?>", m_idx);
	if (pos == string::npos) // 没找到 ?>
	{
		return false;
	}
	m_idx = pos + 2;
	return true;
}

// 解析注释
bool Parser::parse_comment()
{
	if (m_str.compare(m_idx, 4, "<!--") != 0)
	{
		return false;
	}
	m_idx += 4;
	size_t pos = m_str.find("-->", m_idx);
	if (pos == string::npos) // 没找到 -->
	{
		return false;
	}
	m_idx = pos + 3;
	return true;
}

// 解析元素
Xml Parser::parse_element()
{
	Xml elem;
	m_idx++;
	skip_white_space(); // 跳过空格
	const string& name = parse_element_name();
	elem.name(name);
	while (m_str[m_idx] != '\0')
	{
		skip_white_space();
		if (m_str[m_idx] == '/')
		{
			if (m_str[m_idx + 1] == '>')
			{
				m_idx += 2;
				break;
			}
			else
			{
				throw logic_error("xml empty element is error");
			}
		}
		else if (m_str[m_idx] == '>')
		{
			m_idx++;
			string text = parse_element_text();
			if (text != "")
			{
				elem.text(text);
			}
		}
		else if (m_str[m_idx] == '<')
		{
			if (m_str[m_idx + 1] == '/')
			{
				// find the end tag
				string end_tag = "</" + name + ">";
				size_t pos = m_str.find(end_tag, m_idx);
				if (pos == string::npos)
				{
					throw logic_error("xml element " + name + " end tag not found");
				}
				m_idx = (pos + end_tag.size());
				break;
			}
			else if (m_str.compare(m_idx, 4, "<!--") == 0)
			{
				// parse xml comment
				if (!parse_comment())
				{
					throw logic_error("xml comment is error");
				}
			}
			else
			{
				// parse child element
				elem.append(parse_element());
			}
		}
		else
		{
			// parse elem's attr
			string key = parse_element_attr_key();
			skip_white_space();
			if (m_str[m_idx] != '=')
			{
				throw logic_error("xml element attr is error" + key);
			}
			m_idx++;
			skip_white_space();
			string val = parse_element_attr_val();
			elem.attr(key, val);
		}
	}
	return elem;
}

// 解析元素名称
string Parser::parse_element_name()
{
	int pos = m_idx;
	if (isalpha(m_str[m_idx]) || (m_str[m_idx] == '_'))
	{
		m_idx++;
		while (isalnum(m_str[m_idx]) || (m_str[m_idx] == '_') || (m_str[m_idx] == '-') || (m_str[m_idx] == ':') || (m_str[m_idx] == '.'))
		{
			m_idx++;
		}
	}
	return m_str.substr(pos, m_idx - pos);
}

// 解析元素内容
string Parser::parse_element_text()
{
	int pos = m_idx;
	while (m_str[m_idx] != '<')
	{
		m_idx++;
	}
	return m_str.substr(pos, m_idx - pos);
}

// 解析元素属性——键
string Parser::parse_element_attr_key()
{
	int pos = m_idx;
	if (isalpha(m_str[m_idx]) || (m_str[m_idx] == '_'))
	{
		m_idx++;
		while (isalnum(m_str[m_idx]) || (m_str[m_idx] == '_') || (m_str[m_idx] == '-') || (m_str[m_idx] == ':'))
		{
			m_idx++;
		}
	}
	return m_str.substr(pos, m_idx - pos);
}

// 解析元素属性——值
string Parser::parse_element_attr_val()
{
	if (m_str[m_idx] != '"')
	{
		throw logic_error("xml element attr value should be in double quotes");
	}
	m_idx++;
	int pos = m_idx;
	while (m_str[m_idx] != '"')
	{
		m_idx++;
	}
	m_idx++;
	return m_str.substr(pos, m_idx - pos - 1);
}